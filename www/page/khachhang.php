<?php 
    ...
    $checklist = 0;
    if(da_dat_phong($_SESSION['username']))
    {
        $namekhachhang = $_SESSION['username'];
        if(mysqli_query($conn,"SELECT id_phong from dondatphong where username = '$namekhachhang'"))
        {
            $count= mysqli_query($conn,"SELECT id_phong from dondatphong where username = '$namekhachhang'");
            $dataphong = $count->fetch_assoc();
            $idphong = $dataphong['id_phong'];
            if(mysqli_query($conn,"SELECT * from phong where id_phong = ' $idphong'"))
            {
                ...
                $dataphong2 = $count->fetch_all(MYSQLI_ASSOC);
                $checklist = 1;
            }
        }
    }
    ...
    {    
        ...
        if (empty($dataphongbt) && empty($dataphongvip))
        {
            $checklist = 2;
        }
    }
...
<body>
                        ...
                        <ul class="menu-itmes js-menu-itmes">
                            <li class="items-icon items-icon-hover js-items-icon" >
                                ...
                                <ul class="icon-setting-pic js-icon-setting-show bd-rd-rem">
                                    ...
                                    <li class="setting-items"><a href="./logoutkhachhang.php" class="items-link">Log Out</a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
    ...
    <!-- header -->
    <div id="section-content" class="section-content-dashboard">
            <?php if($checklist==1):?>
                <?php 
                foreach($dataphong2 as $phong) : ?>
                        ...
                        <a href="#" class="link-sales">
                                <div class="overview overview-sales bd-rd-rem">
                                    ...
                                    <div class="overview-about mr-16">
                                        <span class="about-content">Phòng đặt: <?= $phong['tenphong']?></span>
                                        <h3 class="about-heading">ĐANG DUYỆT</h3>
                                        <span style="width: 100%;" class="about-time">SDT Hỗ Trợ: 01903181001</span>
                                    </div>
                                </div>
                            </a>
                    </div>
                <?php endforeach; ?>
                <?php elseif ($checklist==0):?> 
                <?php 
                foreach($dataphongvip as $phongvip) : ?>
                    <div class="content-overview">
                        <a href="./datphong.php?idphong=<?=$phongvip['id_phong']?>" class="link-sales">
                                <div class="overview overview-sales bd-rd-rem">
                                    <div class="overview-icon mr-16">
                                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="80px" height="60px" viewBox="0 0 24 24" version="1.1">
                                        <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                    ...
                                    <div class="overview-about mr-16">
                                        <span class="about-content"><?= $phongvip['tenphong']?></span>
                ...
                foreach($dataphongbt as $phongbt) : ?>
                    <div class="content-overview">
                        <a href="./datphong.php?idphong=<?=$phongbt['id_phong']?>" class="link-online">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <h3 style="color: red;" class="head-nuoc">KHÔNG CÒN PHÒNG TRỐNG</h3>
             <?php endif; ?>
        </div>
